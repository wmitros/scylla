    ...
    proceed consume_row_end() {
        ...
        if (_inside_static_row) {
            ...
            if (_slice.options.contains(query::partition_slice::option::reversed) &&
                ...
                // even when `consume_row_start` requested to ignore the row (TODO: explain why this happens).
...
class mx_sstable_mutation_reader : public mp_row_consumer_reader_mx {
    ...
    std::optional<query::partition_slice> _stored_slice;
    const query::partition_slice& _slice;
    Consumer _consumer;
...
public:
    mx_sstable_mutation_reader(shared_sstable sst,
                            ...
                            const query::partition_slice& slice,
                            // If `slice` may die after the constructor (but before the read ends),
                            // `store_slice` tells us to store it.
                            bool store_slice,
                            const io_priority_class& pc,
                            tracing::trace_state_ptr trace_state,
                            streamed_mutation::forwarding fwd,
            ...
            , _stored_slice(store_slice ? std::optional{slice} : std::nullopt)
            , _slice(_stored_slice ? *_stored_slice : slice)
    ...
    future<> advance_context(std::optional<position_in_partition_view> pos) {
        ...
        return [this] {
            ...
            if (_slice.options.contains(query::partition_slice::option::reversed)) {
                ...
                auto inv_pos = position_in_partition_view(pos.region(), new_weight, new_key);

                return get_index_reader().advance_reverse(inv_pos).then([this] {
...
flat_mutation_reader_v2 make_reader(
    ...
    // If we're provided a reversed slice but we're not able to perform a reversed read
    // (reversed multi partition reads are not yet properly handled), we unreverse the slice,
    // the schema (which is provided reversed),
    // and return a mutation fragment stream in the original schema order (non-reversed); the caller
    // is responsible to perform the reversing on their own (as stated in the interface docstring).
    // FIXME: drop this workaround after reversing is fully handled.
    //
    // If we're provided a reversed slice and we *are* able to perform a reversed read,
    // we also fix the slice, because currently callers provide them in a 'half-reversed' format - the order
    // of ranges in the slice is reversed, but the ranges themselves are not.
    // FIXME: drop this workaround when callers are fixed to provide the slice
    // in 'native-reversed' format (if ever).
    if (slice.options.contains(query::partition_slice::option::reversed)) {
        if (range.is_singular()) {
            fixed_slice = legacy_reverse_slice_to_native_reverse_slice(*schema, slice);
        } else {
            fixed_slice = half_reverse_slice(*schema, slice);
    ...
    return make_flat_mutation_reader_v2<mx_sstable_mutation_reader>(
        ...
        fixed_slice ? *fixed_slice : slice, bool(fixed_slice), pc, std::move(trace_state), fwd, fwd_mr, monitor);
}
