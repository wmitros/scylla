...
public:
    ...
    row_processing_result consume_row_start(const std::vector<temporary_buffer<char>>& ecp) {
    ...
    proceed consume_range_tombstone(const std::vector<temporary_buffer<char>>& ecp,
                                            bound_kind kind,
                                            tombstone tomb) {
    ...
    data_consumer::processing_result do_process_state(temporary_buffer<char>& data) {
        ...
        case state::DELETION_TIME_3: {
            ...
            if (ret == mp_row_consumer_m::proceed::no) {
                return mp_row_consumer_m::proceed::no;
            }
        }
        case state::FLAGS:
        flags_label:
        ...
        case state::ROW_BODY_PREV_SIZE:
          {
            ...
            if (ret == mp_row_consumer_m::row_processing_result::retry_later) {
                _state = state::ROW_BODY_PREV_SIZE;
                return mp_row_consumer_m::proceed::no;
            } else if (ret == mp_row_consumer_m::row_processing_result::skip_row) {
                _state = state::FLAGS;
        ...
        row_body_marker_label:
            if (_consumer.consume_row_marker_and_tombstone(
                    _liveness, std::move(_row_tombstone), std::move(_row_shadowable_tombstone)) == mp_row_consumer_m::proceed::no) {
        ...
        case state::NEXT_COLUMN:
            if (!is_column_simple()) {
                ...
                if (_subcolumns_to_read == 0) {
                    ...
                    move_to_next_column();
                    if (_consumer.consume_complex_column_end(column_info) != mp_row_consumer_m::proceed::yes) {
                        _state = state::COLUMN;
                        return mp_row_consumer_m::proceed::no;
                    }
                }
