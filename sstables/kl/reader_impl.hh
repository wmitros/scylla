...
public:
    ...
    mp_row_consumer_k_l(const mp_row_consumer_k_l& c)
        : _permit(c._permit)
        , _trace_state(c._trace_state)
        , _pc(c._pc)
        , _reader(c._reader)
        , _schema(c._schema)
        , _slice(c._slice)
        , _fwd(c._fwd)
        , _range_tombstones(c._range_tombstones)
        , _treat_non_compound_rt_as_compound(c._treat_non_compound_rt_as_compound) {
    }
    ...
    proceed consume_counter_cell(bytes_view col_name, bytes_view value, int64_t timestamp) {
    ...
    proceed consume_cell(bytes_view col_name, bytes_view value, int64_t timestamp, int64_t ttl, int64_t expiration) {
        ...
        case state::ATOM_START_2:
            if (_u16 == 0) {
                ...
                if (_consumer.consume_row_end() ==
                        mp_row_consumer_k_l::proceed::no) {
                    return mp_row_consumer_k_l::proceed::no;
                }
            } else {
