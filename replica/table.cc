...
future<row_locker::lock_holder> table::do_push_view_replica_updates(shared_ptr<db::view::view_update_generator> gen, schema_ptr s, mutation m, db::timeout_clock::time_point timeout, mutation_source source,
    ...
    if (!need_regular && !need_static) {
        ...
        auto permit = co_await gen->get_db().obtain_view_update_reader_permit(s, "push-view-updates-1", timeout, tr_state);
        co_await gen->generate_and_propagate_view_updates(*this, base, permit, std::move(views), std::move(m), { }, tr_state, now, timeout);
    ...
    auto permit = co_await gen->get_db().obtain_view_update_reader_permit(base, "push-view-updates-2", timeout, tr_state);
