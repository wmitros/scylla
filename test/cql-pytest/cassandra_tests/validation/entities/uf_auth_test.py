...
from util import new_aggregate
from test_permissions import grant, revoke, eventually_authorized, eventually_unauthorized
#     @BeforeClass
...
def test_table(cql, test_keyspace):
    ...
    cql.execute("CREATE TABLE " + table + " (k int, v1 int, v2 int, PRIMARY KEY (k, v1))")
    yield table
...
def test_function_in_selection(cql, test_keyspace, test_table):
    with new_simple_function(cql, test_keyspace) as fun:
        cql.execute(f"SELECT k, {fun}() FROM {test_table} WHERE k = 1")
#     @Test
...
def assert_permissions_on_function(cql, user, stmt, fun, args=""):
    assert_unauthorized(cql, user, stmt, fun, args)
    grant_execute_on_function(cql, user, fun, args)
    with new_session(cql, user) as user_session:
        user_session.execute(stmt)
...
def assert_unauthorized(user_session, user, stmt, fun, args):
    with pytest.raises(InvalidRequest, match=f"User {user} has no EXECUTE permission on <function {fun}({args})> or any of its parents"):
        user_session.execute(stmt)
...
def grant_execute_on_function(cql, user, fun, args):
    grant(cql, "EXECUTE", f"FUNCTION {fun}({args})", user)
...
@contextmanager
def new_simple_state_function(cql, keyspace, name=None, args=None):
    body = "(acc int, value int) CALLED ON NULL INPUT RETURNS int LANGUAGE lua AS 'return 0;'"
    with new_function(cql, keyspace, body, args) as fun:
        yield fun
...
@contextmanager
def new_simple_final_function(cql, keyspace, name=None, args=None):
    body = "(input int) CALLED ON NULL INPUT RETURNS int LANGUAGE lua AS 'return input;'"
    with new_function(cql, keyspace, body, args) as fun:
        yield fun
...
@contextmanager
def new_simple_function(cql, keyspace, name=None, args=None):
    body = "() CALLED ON NULL INPUT RETURNS int LANGUAGE lua AS 'return 0;'"
    with new_function(cql, keyspace, body, args) as fun:
        yield fun
