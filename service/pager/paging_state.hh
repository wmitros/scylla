...
private:
    ...
    std::optional<uint32_t> _remaining_high_bits;
    std::optional<uint32_t> _rows_fetched_for_last_partition_high_bits;
...
public:
    paging_state(partition_key pk,
            ...
            std::optional<uint32_t> remaining_ext,
            std::optional<uint32_t> rows_fetched_for_last_partition_high_bits);
    ...
    void set_remaining(uint32_t remaining) {
        _remaining_low_bits = remaining;
        _remaining_high_bits = 0;
    }
    ...
    std::optional<uint32_t> get_remaining_high_bits() const {
        return _remaining_high_bits;
    }

    std::optional<uint32_t> get_rows_fetched_for_last_partition_high_bits() const {
        return _rows_fetched_for_last_partition_high_bits;
    }

    uint64_t get_remaining() const {
        return (static_cast<uint64_t>(_remaining_high_bits.value_or(0)) << 32) | _remaining_low_bits;
    }

    uint64_t get_rows_fetched_for_last_partition() const {
        return (static_cast<uint64_t>(_rows_fetched_for_last_partition_high_bits.value_or(0)) << 32) | _rows_fetched_for_last_partition_low_bits;
    }
